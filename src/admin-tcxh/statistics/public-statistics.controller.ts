...
export class PublicStatisticsController {
  ...
  @ApiOperation({
    ...
    description:
      'API công khai, không cần đăng nhập. Lấy top TNV có điểm cao nhất',
  })
  @ApiQuery({
    name: 'limit',
    required: false,
    type: Number,
    description: 'Số lượng TNV (mặc định 10)',
  })
  ...
  @ApiOperation({
    ...
    description:
      'API công khai, không cần đăng nhập. Lấy top tổ chức có số campaign hoàn thành nhiều nhất',
  })
  @ApiQuery({
    name: 'limit',
    required: false,
    type: Number,
    description: 'Số lượng tổ chức (mặc định 10)',
  })
